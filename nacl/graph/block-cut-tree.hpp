struct block_cut_tree {
  ...
  block_cut_tree(int n)
      : n(n), edges(n + 1), edges_bct(n * 3 + 1),
        edge_id(0) {}
  ...
  void dfs_split(int c, int pid, int &timer,
  ...
  void dfs_build_bct(int c, int g, int &timer,
                     vector<bool> &done) {
    done[c] = true;
  ...
  void build_bct() {
    ...
    for (int i = 1; i <= n; ++i)
      if (!done_dfs[i]) {
        ...
        ++bct_node_cnt;
        dfs_build_bct(i, bct_node_cnt, bct_node_cnt,
                      done_build);
      }
  }
};